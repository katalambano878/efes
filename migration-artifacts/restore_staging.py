#!/usr/bin/env python3
"""Restore Jane's Luxe dump into efes_staging (or janesluxe) on fleet-postgres."""
import json
import os
import subprocess
import sys
...
DUMP = Path(os.environ.get("EF_DUMP_DIR", "/data/coolify/efes-staging/dumps"))
DB = os.environ.get("EF_DB_NAME", "efes_staging")
ROLE = os.environ.get("EF_DB_ROLE", DB)
...
def load_auth_users():
    ...
    n = 0
    for u in users:
        # confirmed_at is GENERATED ALWAYS — never insert it
        cols = [
