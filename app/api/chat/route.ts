...
function buildSystemPrompt(profile: ChatCustomerProfile | null, pagePath?: string): string {
  ...
  let prompt = `You are the AI shopping assistant for this online store. Use the get_website_info tool to find current contact details, address, and policies. Today is ${now}.
  ...
  try {
    ...
    if (!assistantContent) {
      if (ticketCard) {
        ...
        assistantContent = `I'm sorry, I wasn't able to process that properly. You can try rephrasing your request, or visit the Contact page for immediate help.`;
      }
    }
