...
export default function ProductForm({ initialData, isEditMode = false }: ProductFormProps) {
    ...
    const [availabilityType, setAvailabilityType] = useState<'in_store' | 'preorder'>(() =>
        initialData?.metadata?.availability_type === 'preorder' ? 'preorder' : 'in_store'
    );
    ...
    const generateSeoFields = (name: string, desc: string) => {
        const siteName = 'Efescloset';
        const title = name ? `${name} | ${siteName}` : '';
        const metaDesc = desc
            ? (desc.length > 160 ? desc.substring(0, 157).trimEnd() + '...' : desc)
            : name ? `Shop ${name} at ${siteName}.` : '';
        const kw = name
            ? [...new Set([
                name.toLowerCase(),
                        ...
                        <div className="space-y-6 max-w-3xl">
                            ...
                            <div className="border border-gray-200 rounded-xl p-5 bg-gray-50/50">
                                ...
                                <p className="text-xs text-gray-500 mb-4">
                                    In-store items ship from stock. Preorder items appear on the dedicated <span className="font-medium text-gray-700">/preorder</span> page and can show an estimated ship date.
                                </p>
                                <div className="flex flex-col sm:flex-row gap-3">
                                    <button
                                        type="button"
                                        onClick={() => setAvailabilityType('in_store')}
                                        className={`flex-1 text-left px-4 py-3 rounded-lg border-2 transition-all cursor-pointer ${
                                            availabilityType === 'in_store'
                                    ...
                                    >
                                        ...
                                        <span className="text-xs text-gray-500 mt-1 block">Ready to ship from inventory</span>
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => setAvailabilityType('preorder')}
                                        className={`flex-1 text-left px-4 py-3 rounded-lg border-2 transition-all cursor-pointer ${
                                    ...
                                    >
                                        <span className="font-semibold text-gray-900 flex items-center gap-2">
                                            <i className="ri-timer-line text-lg text-amber-700"></i>
                                            Preorder
                                        </span>
                                        <span className="text-xs text-gray-500 mt-1 block">Listed on the Preorder collection page</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            {availabilityType === 'preorder' && (
                                <div>
                                    <label className="block text-sm font-semibold text-gray-900 mb-2">
                                        Estimated shipping / availability
                                    </label>
                                    <input
                                        type="text"
                                    ...
                                    />
                                    <p className="text-xs text-gray-500 mt-1">Shown on the product page so customers know when to expect delivery.</p>
                                </div>
                            )}
                        </div>
                    )}
