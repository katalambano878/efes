...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@efescloset.com';
const EMAIL_FROM = process.env.EMAIL_FROM || 'Efescloset <noreply@efescloset.com>';
const BRAND = {
    name: process.env.NEXT_PUBLIC_SITE_NAME || 'Efescloset',
    color: '#171717',
    colorLight: '#f9fafb',
    colorDark: '#262626',
...
<p style="margin:6px 0 0;color:rgba(255,255,255,0.8);font-size:12px;letter-spacing:1.5px;text-transform:uppercase;">Quality Products</p>
</td></tr>
...
export async function sendSMS({ to, message }: { to: string; message: string }) {
    ...
    try {
        ...
        const response = await fetch('https://api.moolre.com/open/sms/send', {
            ...
            body: JSON.stringify({
                type: 1,
                senderid: process.env.SMS_SENDER_ID || 'Efescloset',
                messages: [
                    {
                        recipient: recipient,
                        message: message
...
`, `Welcome to ${BRAND.name}, ${firstName}!`)
    ...
    if (phone) {
        await sendSMS({
            to: phone,
            message: `Welcome ${firstName}! Thanks for joining us.`
        });
    }
}
